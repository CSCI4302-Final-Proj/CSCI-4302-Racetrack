...
class ImageListener:
    ...
    def imageDepthCallback(self, data):
		...
		l = [ cv_image[x,y] for x  in range(15,25) for y in range(32,48)]
...
#                print(data);
	    ...
	    self.centerDepth = center_depth() # print center depth
            ...
            if self.state == 0 and self.centerDepth < 7200 and self.waitInterval > 40:
                self.msg.vel = -0.08
                self.msg.angle = -0.78
		self.pub.publish(self.msg)
                ...
                self.turnOne = True
                print(self.state)
            elif self.turnOne and self.centerDepth > 5500 and self.exit_turn > 300:
		self.state = 1
                self.turnOne = False
		print("EXITED")
	    ...
	    self.exit_turn += 1
            ...
            if self.state == 1 and self.centerDepth < 6700 and self.waitInterval > 80:
                self.msg.vel = -0.08
                self.msg.angle = -0.78
                self.waitInterval = 0
                self.turnTwo = True
