...
class ImageListener:
    ...
    def imageDepthCallback(self, data):
		...
		l = [ cv_image[x,y] for x  in range(32,38) for y in range(39,41)]
		center_depth = min(l)
...
#		print('Depth at center %d' % center_depth)
	    ...
	    print(self.centerDepth)
            if self.timer > 0 and self.centerDepth < 500:
                if self.turn_right < 40:
			#self.msg.vel = -0.08
                	self.msg.angle = -0.78
                	#self.waitInterval = 0
                	#self.turnOne = True
			...
			self.turn_right += 1
		elif self.turn_right > 39 and self.turn_right < 60:
			#self.msg.vel = -0.08
			self.msg.angle = 0.78
			self.turn_right += 1
			self.turnTwo = True
